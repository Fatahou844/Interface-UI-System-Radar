...
namespace Interface_RADAR
{
    public class Radar
    {
        ...
        void CreateBaseImage()
        {
            ...
            //g.DrawEllipse(p_yellow, (_size - interval) / 2, (_size - interval) / 2, interval, interval);
            g.DrawArc(p_yellow, (_size - interval) / 2, (_size - interval) / 2, interval, interval, 0, -180);
            g.DrawString("15 m", drawFont, drawBrush, new Point((int)((_size) / 2), (_size - interval) / 2));
         
            interval = _size * 1/2;
            g.DrawArc(p_org, (_size - interval) / 2, (_size - interval) / 2, interval, interval, 0, -180);
            //g.DrawEllipse(p_org, (_size - interval) / 2, (_size - interval) / 2, interval, interval);
